...
It reports three things: `STRUCTURE_SEGMENT_OUT_OF_ORDER` (the sequence stops being derivable from the published order, read with how often each segment occurs set aside), `STRUCTURE_SEGMENT_CARDINALITY` (fewer occurrences than the published minimum along the path from the structure root, or more than the published maximum), and `STRUCTURE_SEGMENT_UNEXPECTED` (a segment the publication does not name, a `Z` segment included, at `warning` severity). Every finding carries a **PHI-free locus**: a segment name, a 0-indexed occurrence and a published structure id, never a field value.

The three questions are kept apart so one defect is reported once, but the order check keeps the publication's **group structure**: a group the publication lets repeat may repeat, which is what lets a conformant `ORU^R01` carry `OBR OBX OBR OBX`, while a group bounded at one occurrence may not be re-entered, so a `VXU^V04` whose `PV2` arrives before its `PV1` is reported.

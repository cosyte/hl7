...
- 45c5b78: Added a datetime conversion surface: `toObject`, `toISO` and `toDate`.
  ...
  The offset a value states is bounded on the same terms as its day: a whole number of minutes within 23 hours 59 minutes of UTC, which is both what the `+HH:MM` rendering can state and what an ISO-8601 reader accepts, since `new Date("2024-02-29T12:00:00+24:00")` is an `Invalid Date`. Anything else names no zone, so all three answer `undefined` rather than reporting a string in a slot documented as a number of minutes east of UTC, rendering `+NaN:NaN` or a three-digit hour into a two-digit slot, or multiplying a `null` or a `"0"` to zero and handing back a confident UTC instant. Two routes reach that field without a cast: a hand-built value, since the parsed type is a structural interface, and `msg.meta.timestamp`, whose lenient fallback read range-checks the calendar components and not the offset, so a message timestamp of `2024-02-29T12:00:00+99:99` states 6039 minutes east and used to render as `+100:39`. Every zone the world keeps is far inside the bound, the widest in use being 14 hours east; a stated offset still wins outright over any assumption beside it; and an `offsetMinutes` on a value that claims no zone states nothing, so it is ignored rather than refused. `parseDtm` still accepts what it always accepted, up to `+2400`, and `formatDtm` and `dtmToDate` still answer for it: the refusal belongs to the conversion, exactly as the calendar bound does.

  `toDate` is equally careful about the argument it is handed rather than only the value: the options bag may be `null` or absent, and `assumeOffsetMinutes` has to be a finite number of minutes or the answer is `undefined`. From JavaScript, where the type is not checked for you, `"0"`, `true` and `[]` all multiply to `0`, so an unguarded delegation would hand back a confident UTC instant nobody asked for, which is the guess this surface exists to refuse; an offset large enough to leave the range a `Date` can represent is refused the same way, so no answer is ever an `Invalid Date`. A value carrying its own offset is untouched by this: its offset still wins outright and the assumption beside it is ignored, not treated as an error. `dtmToDate` keeps the behaviour it has always had; the check belongs to the new surface.
...
- 848c7e7: `msg.allergies()` now reads `IAM` (Patient Adverse Reaction Information) segments as well as `AL1`, so an ADT^A60, or any message that carries its allergies in `IAM`, no longer returns an empty list that reads as "no allergies recorded".
...
- 08a6864: **This is 0.1.0, the first release of `@cosyte/hl7` whose public API we treat as settled.**
...
- cbee588: `msg.medications()` now lets you tell a new pharmacy order from a discontinued, held or cancelled one: every medication carries `orderControl`, the ORC-1 order control of the `ORC` that opened its order group, exactly as the sender wrote it.
...
- 81dfc86: Vendor date formats gain month names, AM/PM, 12-hour clocks and single-digit tolerance, and `dateFormats` validation is now strict: a format that previously constructed and then never matched will now throw where you wrote it.
...
- 086ae19: Tooling: the numbers the release readiness record states are now recounted from the tree instead of read as prose.
...
- 41e1a3c: Docs: the first example in the README and the first example in the quickstart are now executed by the test suite, read straight out of the page they are printed on.
...
- 47b20f5: Repository tooling: the em-dash gate and the public-surface gate now run as thin callers of the shared reusable workflows in `cosyte/.github`, pinned to an immutable commit.
...
- 3760216: Repository tooling: the public-surface gate now reaches its detection through the published `@cosyte/script-utils` internal-reference implementation instead of this repository's own copy of it.

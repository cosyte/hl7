...
describe("model/dot-path: resolvePath — acceptance paths", () => {
  ...
  it("resolves PID.5 to the first subcomponent of the first component", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("PID.5", msg.rawSegments, msg.encodingCharacters)).toBe("Smith|Jr");
  });
  ...
  it("resolves PID.3[0].1 to component 1 of the first repetition", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("PID.3[0].1", msg.rawSegments, msg.encodingCharacters)).toBe("100");
  });

  it("resolves PID.3[1].1 to component 1 of the second repetition", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("PID.3[1].1", msg.rawSegments, msg.encodingCharacters)).toBe("200");
  });
  ...
  it("resolves MSH.1 to the field separator character `|`", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("MSH.1", msg.rawSegments, msg.encodingCharacters)).toBe("|");
  });

  it("resolves MSH.2 to the encoding-characters string `^~\\&`", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("MSH.2", msg.rawSegments, msg.encodingCharacters)).toBe("^~\\&");
  });

  it("resolves MSH.12 to the HL7 version string", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("MSH.12", msg.rawSegments, msg.encodingCharacters)).toBe("2.5");
  });

  it("returns undefined for a missing segment (MODEL-05)", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("NOT.9.9", msg.rawSegments, msg.encodingCharacters)).toBeUndefined();
  });

  it("returns undefined for an out-of-range field", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("PID.99", msg.rawSegments, msg.encodingCharacters)).toBeUndefined();
  });

  it("returns undefined for an out-of-range component", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("PID.5.99", msg.rawSegments, msg.encodingCharacters)).toBeUndefined();
  });

  it("returns undefined for an out-of-range segment occurrence", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("OBX[5].5", msg.rawSegments, msg.encodingCharacters)).toBeUndefined();
  });

  it("returns undefined for an out-of-range repetition", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("PID.3[5].1", msg.rawSegments, msg.encodingCharacters)).toBeUndefined();
  });
  ...
  it("resolves PID.5.2 to the second component (Jane)", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("PID.5.2", msg.rawSegments, msg.encodingCharacters)).toBe("Jane");
  });

  it("resolves PID.3[1].4 to the 4th component of PID-3 rep[1] (MRN)", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("PID.3[1].4", msg.rawSegments, msg.encodingCharacters)).toBe("MRN");
  });

  it("resolves PID.8 (gender field)", () => {
    const msg = parseHL7(FIXTURE);
    expect(resolvePath("PID.8", msg.rawSegments, msg.encodingCharacters)).toBe("F");
  });
});

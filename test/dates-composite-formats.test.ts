 ...
 * typed datetime. `BUILTIN_DATE_FALLBACKS` is a US-order list carrying
 * `MM/DD/YYYY` and no day-first form; it is the library reading a sender the
 * caller never described, and it does not do that on a clinical datetime. In a
 * PHI-bearing library a plausible wrong date of birth is a patient
 * misidentification, and re-running the parse does not undo what the consumer
 * already wrote. So the composite path tries the strict HL7 DTM, then the
 * declared formats, and then stops.
 *
 * `msg.meta.timestamp` is deliberately NOT part of that rule: it is a
 * non-composite caller that has always run the full lenient cascade. Its
 * behaviour here is pinned as a boundary, not as a target. One visible
 * consequence of the boundary, asserted below: the order-ambiguity refusal is
 * produced BY the built-in stage, so `msg.meta.timestamp` answers an
 * undeclared `05/07/1988` with an `ambiguity` report while a typed datetime
 * field answers with a plain `valid: false` and no report, having consulted
 * nothing that could offer a second reading.
 */

import { describe, expect, it } from "vitest";

import { AMBIGUOUS_DATE_ORDER, parseHL7, type ParseOptions, type TS } from "../src/index.js";
...
describe("dates/composite: built-in fallbacks never reach a typed datetime (AC-4)", () => {
  ...
  it("no built-in stage means no order-ambiguity report on a typed field either", () => {
    // The refusal is produced BY the built-in stage: `MM/DD/YYYY` matches, the
    // day-first probe also matches, and the two readings disagree. A typed
    // datetime field never reaches that stage, so it reports a plain invalid
    // with no `ambiguity` key at all: nothing offered a second reading. MSH-7
    // does reach it, and carries the report. Both halves are the same rule.
    const msg = parseHL7(clinicalMessage("05/07/1988"));
    const dob = msg.segments("PID")[0]?.field(7).asTs();
    expect(dob).toEqual({ raw: "05/07/1988", valid: false, hasTimezone: false });
  ...
  it("declaring the order resolves the typed field and reports no ambiguity", () => {
    const msg = parseHL7(clinicalMessage("05/07/1988"), { dateFormats: ["DD/MM/YYYY"] });
    const dob = msg.segments("PID")[0]?.field(7).asTs();
    expect(dob).toMatchObject({ valid: true, month: 7, day: 5, matchedFormat: "DD/MM/YYYY" });
    expect(dob?.ambiguity).toBeUndefined();

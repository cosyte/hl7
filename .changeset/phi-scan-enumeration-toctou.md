...
The PHI gate no longer lets a build tool's transient file refuse an entire scan: a file the scanner
enumerated itself, that git does not track, and that vanishes before the read is now skipped and
reported instead.
...
and it is written to stderr as skipped rather than dropped silently.

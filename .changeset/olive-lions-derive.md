...
**What consumers will notice: warning volume rises on non-conformant traffic.** More trigger events are recognized, so messages that used to be `recognized: false` are now checked. More segments are expected per pair, most visibly `EVN` across the whole ADT family (the previous table excluded it deliberately, which is exactly what stopped the library reporting a genuinely missing required segment) and `PV1` in `MDM`. And an `ORU^R01` carrying `OBX` but no `OBR` now warns, where the old anchor accepted either. If your channel treats a `MISSING_EXPECTED_GROUP` warning as a rejection, sample your traffic before upgrading.

...
One input strict now **rejects** that it accepted before: a hand-rolled profile object keyed with a lowercase `customSegments` name, parsed against a matching lowercase segment. It claimed the segment then and still does, so lenient parsing is unaffected, but the segment is miscased and strict now says so.

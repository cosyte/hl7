...
Of those four, the adjacency rule is also the one the matcher applies: a format whose numeric tokens meet with nothing between them reports no match, rather than answering a digit run that has two equally correct readings with one of them. The other three describe a format nobody can honour, so refusing it where it is written is enough.

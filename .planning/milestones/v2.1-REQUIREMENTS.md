# @cosyte/hl7 — v2.1 Requirements (archived)

> **Archive note (2026-04-30):** This file is the requirement set as it stood at v2.1 milestone close. The package was renamed mid-milestone from `@cosyte/hl7-parser` to `@cosyte/hl7` (Phase 9, 2026-04-20); the legacy name appears below where it was written into the original document — read `@cosyte/hl7` everywhere it shows up. All 97 v1 REQ-IDs are checked off; outcomes captured in `v2.1-ROADMAP.md`.

All requirements are user-facing behaviors a developer consuming `@cosyte/hl7` can verify. REQ-IDs are stable across phases and were referenced from `ROADMAP.md` for traceability throughout v2.1.
